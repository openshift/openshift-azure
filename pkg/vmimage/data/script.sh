...
# install golang-1.12 from source because rhel7 ships with golang 1.9 by default
curl -s https://storage.googleapis.com/golang/go1.12.5.linux-amd64.tar.gz --output /tmp/go1.12.5.linux-amd64.tar.gz
echo "aea86e3c73495f205929cfebba0d63f1382c8ac59be081b6351681415f4063cf /tmp/go1.12.5.linux-amd64.tar.gz" | sha256sum -c
tar -xzf /tmp/go1.12.5.linux-amd64.tar.gz -C /usr/local
export GOPATH=/go
...
set -x
azure tlsproxy --insecure --key client-key.pem --cert client-cert.pem --hostname https://cdn.redhat.com/ &
...
go get github.com/openshift/openshift-azure/cmd/vhdfooter
vhdfooter -size $((DISKGIB << 30)) >>/var/lib/libvirt/images/$IMAGE.vhd
