...
func TestReadUpdateBlob(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if (err != nil) != (tt.wantErr != nil) {
			t.Errorf("Upgrade.readUpdateBlob() error = %#v, wantErr %#v", err, tt.wantErr)
			return
		}
		...
		if tt.wantErr == nil && !reflect.DeepEqual(got, tt.want) {
			t.Errorf("Upgrade.readUpdateBlob() = %#v, want %#v", got, tt.want)
		}
	}
}
...
func TestWriteUpdateBlob(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if err := u.Write(tt.blob); (err != nil) != (tt.wantErr != "") {
			t.Errorf("Upgrade.writeUpdateBlob() error = %v, wantErr %v", err, tt.wantErr)
		}
	}
}
