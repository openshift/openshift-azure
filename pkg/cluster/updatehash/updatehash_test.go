...
func TestHashScaleSets(t *testing.T) {
	tests := []struct {
		...
		exp  map[ScalesetName]updateblob.Hash
	}{
		{
...
func TestInitialize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := i.Initialize(tt.cs, map[string]interface{}{}); (err != nil) != tt.wantErr {
...
func TestDeleteAllBut(t *testing.T) {
	tests := []struct {
		...
		expectNeedsSaving bool
	}{
		{
			name:              "delete none",
			expectNeedsSaving: false,
			beforeBlob: updateblob.Updateblob{
				"ss-compute_0": "",
				"ss-compute_1": "",
				"ss-compute_2": "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if i.needsSaving != tt.expectNeedsSaving {
				t.Errorf("expected needsSaving:\n%#v\ngot:\n%#v", tt.expectNeedsSaving, i.needsSaving)
			}
		})
	}
}
