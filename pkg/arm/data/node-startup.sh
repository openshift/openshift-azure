...
cat >/etc/origin/node/node-config.yaml <<'EOF'
apiVersion: v1
authConfig:
...
kubeletArguments:
  ...
  node-labels:
  - node-role.kubernetes.io/{{ .Extra.Role }}=true
  pod-manifest-path:
  ...
  - 10m
  kube-reserved:
  - {{ .Derived.KubeReserved .ContainerService .Extra.Role | quote }}
  system-reserved:
  - {{ .Derived.SystemReserved .ContainerService .Extra.Role | quote }}
