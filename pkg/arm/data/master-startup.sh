...
if [[ "$(hostname)" == "master-000000" ]]; then
  ...
  while ! curl -o /dev/null -m 2 -fs https://master-000000/healthz; do
    sleep 1
  done
  ...
  # TODO: duplication of pkg/addons/data/Namespace/openshift-sdn.yaml
  oc create -f - <<'EOF'
apiVersion: v1
kind: Namespace
metadata:
...
EOF
  ...
  # TODO: duplication of pkg/addons/data/ServiceAccount/openshift-sdn/sdn.yaml
  oc create -f - <<'EOF'
apiVersion: v1
kind: ServiceAccount
metadata:
...
EOF
  ...
  oc adm policy add-scc-to-user privileged system:serviceaccount:openshift-sdn:sdn

  # TODO: duplication of pkg/addons/data/ClusterRoleBinding.rbac.authorization.k8s.io/sdn-reader.yaml
  oc create -f - <<'EOF'
...
EOF
  ...
  # TODO: duplication of pkg/addons/data/ClusterRoleBinding.rbac.authorization.k8s.io/sdn-cluster-reader.yaml
  oc create -f - <<'EOF'
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRoleBinding
metadata:
