...
func (g *simpleGenerator) vnet() *arm.Resource {
	...
	return &arm.Resource{Resource: vn}
}

func (g *simpleGenerator) ipAPIServer() *arm.Resource {
	...
	return &arm.Resource{Resource: &pipa}
}

func (g *simpleGenerator) ipOutbound() *arm.Resource {
	return &arm.Resource{
...
func (g *simpleGenerator) lbAPIServer() *arm.Resource {
	...
	return &arm.Resource{Resource: lb}
}

func (g *simpleGenerator) ilbAPIServer() *arm.Resource {
	return &arm.Resource{
...
func (g *simpleGenerator) lbKubernetes() *arm.Resource {
	return &arm.Resource{
		Resource: &network.LoadBalancer{
			Sku: &network.LoadBalancerSku{
				Name: network.LoadBalancerSkuNameStandard,
...
func (g *simpleGenerator) storageAccount(name string, tags map[string]*string) *arm.Resource {
	return &arm.Resource{
...
func (g *simpleGenerator) nsgMaster() *arm.Resource {
	return &arm.Resource{
...
func (g *simpleGenerator) nsgWorker() *arm.Resource {
	return &arm.Resource{
		Resource: &network.SecurityGroup{
			SecurityGroupPropertiesFormat: &network.SecurityGroupPropertiesFormat{
				SecurityRules: &[]network.SecurityRule{},
...
func (g *simpleGenerator) Vmss(app *api.AgentPoolProfile, backupBlob, suffix string) (*compute.VirtualMachineScaleSet, error) {
	...
	rr, ok := r.Resource.(*compute.VirtualMachineScaleSet)
	if !ok {
		return nil, errors.New("can not convert value into VirtualMachineScaleSet")
	}

	return rr, nil
}

func vmss(cs *api.OpenShiftManagedCluster, app *api.AgentPoolProfile, backupBlob, suffix string, testConfig api.TestConfig) (*arm.Resource, error) {
	...
	return &arm.Resource{Resource: vmss}, nil
}
