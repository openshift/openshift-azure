...
func NewPlugin(log *logrus.Entry, pluginConfig *pluginapi.Config, testConfig api.TestConfig) (api.Plugin, []error) {
	return &plugin{
		...
		configGenerator: config.NewSimpleGenerator(testConfig),
	}, nil
}
...
func (p *plugin) initialize(ctx context.Context, oc *api.OpenShiftManagedCluster) error {
	var err error
	if p.kubeclient == nil {
		p.kubeclient, err = kubeclient.NewKubeclient(p.log, oc.Config.AdminKubeconfig, false, p.testConfig)
	}
	return err
}
...
func (p *plugin) GetPluginVersion(ctx context.Context) ([]byte, error) {
	pluginVersion := &struct {
		...
		PluginVersion: p.pluginConfig.ClusterVersion,
	}
