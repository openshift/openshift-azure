...
OpenShift on Azure project is using few testing methods. 
...
/test build - run container build tests
...
Packages under `pkg` directory contains their individual unit tests.
To run all unit tests locally, execute: `make unit`.
To run individual test:
`go test github.com/openshift/openshift-azure/pkg/tls`

## OpenShift E2E tests

Project has its own End-2-End testing test suite. You can find it under:
`test/e2e/`. It uses [ginko](https://github.com/onsi/ginkgo) and [gomega](https://github.com/onsi/gomega).
To run E2E tests locally, execute: `make e2e`

## OpenShift Origin Conformance tests

For the generic OpenShift functionality we use [OpenShift Origin](https://github.com/openshift/origin)
